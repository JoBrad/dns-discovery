...
| 003 | tls-health-check | standard | VALIDATED ✓ | TLS Health Checks | Stdlib `ssl` sufficient; OpenSSL codes distinguish cert failure modes |
...
- **TLS**: Python stdlib `ssl` (no external dependency)
- **Pattern Matching**: Python stdlib `re`
- **Python version**: 3.14.2 (with asyncio.Runner support)
...
3. **TLS Health** — Stdlib `ssl` sufficient for all scenarios
   - `ssl.create_default_context()` performs full chain validation
   - OpenSSL verify_code mapping: 10=expired, 18/19=self-signed, 62=hostname mismatch
